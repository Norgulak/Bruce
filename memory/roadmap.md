...
- **Post-interrupt audio dropped, causing frequent mishearing (~40% per Banmi, reported 2026-08-14)** — traced through `bruce.py`'s `voice_interrupt_listener()`, `convo_thread()`, and `record_until_silence()`. When Banmi interrupts Bruce mid-response, the interrupt is detected by a separate mic stream that only measures volume and throws away the actual audio. The real recording (`record_until_silence()`) only opens a brand-new mic stream *after* the interrupt is fully processed — a gap of roughly 100-300ms (stream teardown/setup + thread polling delay) during which anything Banmi is already saying is never captured. Confirmed by his example: interrupting to ask "why did you bring up a YouTube video" was transcribed as just "me a YouTube video" — the first ~2/3 of the sentence was said into the discard-only interrupt stream. This is specific to interrupt-triggered turns (normal, non-interrupted back-and-forth doesn't have this gap) but interrupting mid-response is common enough that it likely accounts for a large share of the ~40%. Real fix is architectural, not a quick patch: stop using two disconnected mic streams for "detect interrupt" vs. "record what comes next" — either share one continuous stream, or have the interrupt listener hand off a short pre-roll buffer of audio into the new recording so nothing said right at the start is thrown away. Bigger, riskier change than tonight's other fixes (core audio pipeline) — needs its own branch and real hardware testing, not a rushed patch. **[proposed, not started, high priority — directly causes reported conversation quality issue]**
