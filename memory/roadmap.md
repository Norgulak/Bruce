...
3. Streaming TTS to fix the ~5s response delay. Also listed under Pending Fixes — same underlying problem. **[confirmed, LEANING AWAY FROM LiveKit — see below]** Found and verified 2026-08-14 (reel): `kyutai-labs/pocket-tts` (github.com/kyutai-labs/pocket-tts, 7.9k stars, actively maintained) is a strong candidate to replace Kokoro directly instead of adopting LiveKit's full WebRTC stack. CPU-only by design (Kyutai tested GPU, no speedup at their batch size), 100M params, ~200ms to first audio chunk, plain `pip install pocket-tts` + simple Python API (`from pocket_tts import TTSModel`), has a `serve` mode that keeps the model warm in memory via local HTTP — could run as a background service Bruce POSTs text to. Also supports voice cloning from a WAV file (their usage terms prohibit cloning someone's voice without consent — fine for a stock preset or Banmi's own voice, not for cloning e.g. a celebrity). Not yet integrated or tested on Banmi's hardware — next real candidate for this roadmap slot, pending Banmi's go-ahead to start.
